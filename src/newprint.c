...
/*
 * The decompose() function decomposes a pseudonym "equation" into individual
 * terms. Pseudonym calculus is a pretty weird concept, but to put it
 * plainly, decompose() takes an equation like:
 * 
 * (a^b)+c+(d^f)+e
 * 
 ...
 * 
 * a     c     d     e    <- actual authors --- + --- + --- + --- b           f
 * <- pseudonyms
 * 
 * The authors (as published) can be found by removing the elements to the left
 * of each ^ sign:
 * 
 * (Neal Stephenson+George Jewsbury)^Stephen Bury = Stephen Bury
 * 
 * (Alexei Panshin^Louis J. A. Adams)+Joe L. Hensley = Louis J. A. Adams + Joe
 * L. Hensley
 * 
 * The actual authors can be found by removing the elements to the right of each
 * ^ sign:
 * 
 * (Neal Stephenson+George Jewsbury)^Stephen Bury = Neal Stephenson + George
 * Jewsbury
 * 
 * (Alexei Panshin^Louis J. A. Adams)+Joe L. Hensley = Alexei Panshin + Joe L.
 * Hensley
 */
auset_t        *
...
/*
 * This function is identical to strlen(), but doesn't include embedded html
 * directives. (Calculates the screen length of an html string).
 */
...
int
posit_check(position, rightmargin, indent)
{
	int             loop;
...
read_and_printwrap(int filetype, char *coffset, int position, int rightmargin, int indent)
{
	...
	if (fp != NULL) {
		...
		while (input != '|') {
			...
			/*
			 * If we word wrap during italics, identation will be
			 * wrong since the italics may have a different font
			 * width than the regular font. This code figures out
			 * when we are doing italics.
			 */
			if (input == '<') {
				skipping = 1;
				input = getc(fp);
			...
			/*
			 * Don't count the length of html directives since
			 * the don't appear on screen.
			 */
			if (!skipping)
				position++;
...
/*
 * html_print1_author() is used to output an author hotlink. It should be
 * handed and author/pseudo string pair following a decompose/separate
 * operation in the AE field.
 * 
 * o position should be set to the current cursor position. o rightmargin should
 * be set to the offset where wordwrap is desired. o ident should be set to
 * the offset where output should continue following a wordwrap.
 * 
 */
html_print1_author(
		   char *author,
		   ...
		   int indent)
{
	...
	/*
	 * Make a copy of the author string, altering all of the spaces to
	 * underscores. This version of the author is used as an argument to
	 * exact_author embedded in the html.
	 */
	...
	/*
	 * If this was a collaboration as a pseudonym, start a left paren, so
	 * we get: (a and b) (as c).
	 */
	ptr1 = author;
	...
	/*
	 * If this was a collaboration, output each author involved with the
	 * work.
	 */
	while (strstr(ptr1, "+")) {

		/*
		 * Get the underscored and non-underscored versions of the
		 * author string in sync.
		 */
	...
	while (tmp) {
		if (strcmp(tmp->au_actual, exact_author)) {
			...
			if (strstr(tmp->au_actual, "+") && strstr(tmp->au_actual, exact_author)) {
				...
				/*
				 * The author has collaborated as a
				 * pseudonym. Strip out the author's name.
				 */
				newactual[0] = 0;
...
/*
 * This is the central title printing function. It combines several different
 * printing styles into one function, which explains its needless complexity.
 * It's easier understood if followed using a single option type. Recursion
 * is used to handle variant titles.
 */
print_title(int indent, char *title, int rightedge, int options, char *offset)
{
	...
	/*
	 * If a variant title is present, then recursively call print_title()
	 * on each subtitle.
	 */
	if (strstr(title, "^")) {
		char           *vptr;
		...
		*vptr = 0;
		 ...
		 * T_VARIANT option, so that the title is annotated as: [vt
		 * title]. Separate each title with a newline.
		 */
		title = vptr + 1;
		...
		/*
		 * Last: output the last variant title. We only care about
		 * the cursor position on the last title, so that's the one
	 ...
	 * options:
	 * 
	 * T_PREINDENT = we aren't allowed to prepend any characters to the
	 * title. T_WINNER    = prepend an 'o' to the title.
	 * 
	 * otherwise pad the title with the spaces indicated by ident.
	 */
	...
	/*
	 * If this is a variant title, start a variant annotation. Note that
	 * this can be used with the T_QUOTE or T_NOPAD options, so don't
	 * else it.
	 */
	...
	/*
	 * The T_NOPAD is used to italize novels, while the T_QUOTE is used
	 * to quote short fiction.
	 */
	if (options & T_QUOTE) {
		printf("\"");
...
again:
	if (titlelen > rightedge) {
		...
		/*
		 * If we know in advance that the title is going to spill off
		 * the right margin, then go ahead and calculate where to
		 * break the title.
		 */
		...
		if (loop) {
			...
			/*
			 * Output the characters up to the title break point.
			 */
			...
			/*
			 * Put a kabosh an the italics, and generate a
			 * newline. (The space character is the wrong width
			 * if italicized)
			 */
			...
			/*
			 * Now ident the appropriate amount. Move in 2 more
			 * spaces to denote a wrap.
			 */
			...
			/*
			 * Figure out where we left off and start again.
			 */
			...
			/*
			 * Sorry, but the title couldn't be broken up. If an
			 * offset is present, then make the title a hotlink
			 * to a publication citation.
			 */
			if (offset) {
				printf("<a href=\"http:/%s/pwork.cgi?%s\">",
				       HTFAKE, offset);
		...
		/*
		 * The title (or last remainder of a broken title) fits
		 * without being broke up.
		 */
		if (offset) {
			printf("<a href=\"http:/%s/pwork.cgi?%s\">",
			       HTFAKE, offset);
	...
	/*
	 * The title is all out now. Finish up the option handling. First,
	 * finish variant title citations.
	 */
	if (options & T_VARIANT) {
		/*
		 * We can't output any trailing characters if T_NOPAD is set,
	     ...
	     char *seriesnum)
{
	...
	/*
	 * TITLE - Output the title portion of the citation. This includes
	 * the displaying of variant titles, if present.
	 */
	if (corrections) {
	...
	/*
	 * DATE - Output the year of the citation. May require decoding if
	 * the date is forthcoming (9999...), unknown (0), or unpublished
	 * (8888).
	 */
	...
	/*
	 * Under "special" circumstances, shortfiction and collections may
	 * not be displayed in the appropriate section (as when they wind up
	 * in the series section). Tag these citations as special.
	 */
	if (special) {
	...
	/*
	 * AWARDS - List any awards nominations or wins. Note that since this
	 * is the last item printed in the citation, wordwrap checks should
	 * be done before printing, and not after, to prevent the
	 * introduction of blank lines into the output.
	 */
...
/*
 * dotype: 0 = fiction 1 = anthologies 2 = nonfiction 3 = essays
 */
print_series(search_t * list, char *exact_author, int corrections, int dotype)
{
	...
	while (foundseries) {
		...
		while (tmp) {
			...
			/*
			 * If the entry has a series, but we aren't tracking
			 * one, start tracking one.
			 */
			...
			if (tmp->se_series && strcmp(tmp->se_series, current_series) == 0) {
				...
				/*
				 * If the entry has a superseries, but it
				 * doesn't match the current one, skip it.
				 */
		 ...
		 * subseries, kill the superseries, and pass through the list
		 * again.
		 */
...
print_pub_citations(char *citations, int correction)
{
	...
	/*
	 * Use tmp2 to point to the remnants of the old publication list,
	 * then set pub_list to point at the subset we created. Then sort the
	 * subset.
	 */
	tmp2 = pub_list;
	pub_list = pub_list2;
...
print_fatal(char *msg)
{
	...
	case 0:
		...
		break;
	case 1:
		printf("(Truncated Line)\n");
		printf("Chars parsed=[%d]\n", errcount);
		printf("Field parsed=[%s]\n", pstring);
		break;
	case 2:
		printf("(Field Too Large)\n");
		printf("Chars parsed=[%d]\n", errcount);
		printf("Field parsed=[%s]\n", pstring);
		break;
	}
...
lastname(char *fullname)
{
	...
	while ((unsigned int) tmp > (unsigned int) fullname) {
		if (*tmp == ' ') {
			...
			/*
			 * This section does special handling for lastnames
			...
			if (strncmp(tmp, " Le ", 4) && strstr(fullname, " Le ")) {
				...
				/*
				 * This section does special handling for
				 * stuff which follows the last name.
				 */
...
print_titles_author(search_t * list)
{
	...
	while (tmp) {
		...
		/*
		 * Put the title on the author's list. Put at the end of the
