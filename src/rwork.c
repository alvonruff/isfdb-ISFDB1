...
static char sccsid[] = "@(#)rwork.c	1.4	01/28/98 SFdbase";
	...
	char	*argv[];
{
	...
	/*
	 * Load in the target entry. Then load in the
	 * reviews for that author.
	 */
	parse_work( argv[1] );
	load_reviews(F_TAUTHOR, title_list->se_author );

	/*
	 * Now look through the list for title matches, and
	 * fill in a publication citation list.
	 */
	publist[0] = 0;
	rtmp = rv_list;
	if (rtmp == NULL) {
		printf("<html><head>\n");
		printf("<title>ISFDB Internal Error</title></head>\n");
		printf("<body bgcolor=#ffffff>\n");

		printf("<h1>ISFDB Internal Error</h1><hr>\n");
		printf("Can't find <i>that</i> review.\n");
		printf("<pre>\n");
		postamble("rwork", argv[1]);
		printf("</pre>\n");
		exit(1);
	}
	...
	while (rtmp) {
		...
		if (strstr(title_list->se_title, "^" )) {
			if (strstr(title_list->se_title, rtmp->rv_title )) {
				doit = 1;
			}
		} else if (strcmp(title_list->se_title, rtmp->rv_title ) == 0) {
				doit = 1;
		}
		if (doit) {
			if (publist[0]) {
			...
			strcat(publist, rtmp->rv_abbreviation);
		}
		rtmp = rtmp->rv_next;
	}
	...
	printf("<html><head>\n");
	printf("<title>Review History</title></head>\n");
	printf("<body bgcolor=#ffffff>\n");
	printf("<h1>Review History</h1><hr>\n");
	print_menus2( argv[1] );
	printf("<hr>\n<pre>\n");
	...
	load_pubs(F_PLIST, publist );
	printf("</pre>\n");
	print_pub_citations( publist, 0 );
	printf("<pre>\n");
	postamble("rwork", argv[1]);
