...
static char sccsid[] = "@(#)titledups.c	1.3	02 May 1997 SFdbase";
...
find_dups()
{
	...
	while(current) {
		...
		while(probe) {
			...
			} else if (strstr(current->rc_title, ":") && probe->rc_series) {
				char *ptr1, *ptr2;
				char bogus1[128];
				char bogus2[128];
			...
			} else if (strstr(probe->rc_title, ":") && current->rc_series) {
				char *ptr1, *ptr2;
				char bogus1[128];
				char bogus2[128];
...
find_dups2(char *optyear)
{
	...
	while(current) {
		...
		while(probe) {
			if (optyear) {
				if (strcmp(current->rc_year, optyear)) {
					break;
				}
			}
			if (similar(current->rc_title, probe->rc_title, current->rc_lowt, probe->rc_lowt)) {
...
#ifndef NEWSTUFF
				if ( !similar(current->rc_author, probe->rc_author, 
					      current->rc_lowa, probe->rc_lowa)) {
					probe = probe->rc_next;
					continue;
				}
