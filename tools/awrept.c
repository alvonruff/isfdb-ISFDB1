	...
	while(newdb) {
		...
		while(isfdb) {
			if (isfdb->rc_awtag == NULL) {
				...
				if ( strcmp(isfdb->rc_title, newdb->rc_title)) {
					if (first) {
						first = 0;
						printf("==================================================\n");
					}
					printf("ISFDB (title): %s\n", isfdb->rc_title);
					printf("AWARD (title): %s\n", newdb->rc_title);
				}
				if ( strcmp(isfdb->rc_author, newdb->rc_author)) {
					if (first) {
						first = 0;
						printf("==================================================\n");
					}
					printf("ISFDB (author): %s\n", isfdb->rc_author);
					printf("AWARD (author): %s\n", newdb->rc_author);
				}
				break;
			}
			isfdb = isfdb->rc_next;
